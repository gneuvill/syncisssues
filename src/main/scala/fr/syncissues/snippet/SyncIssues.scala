...
class SyncIssues extends Observing {
  ...
  type IActor = SpecializedLiftActor[Seq[(IPServ, Task[Seq[Issue]])]]

  type PActor = SpecializedLiftActor[Seq[Task[Vector[Project]]]]
  ...
  implicit object IssueOrdering extends scala.Ordering[Issue] {
    ...
    def messageHandler = {
      case s => showWork {
        ...
        projects() = dummyProject :: (commonProjects {
          s map (_.attemptRun match {
            case \/-(prj) => prj
            case -\/(t) => t.printStackTrace; dummyProject :: Nil
          })
  ...
  def syncIssues(srv1: IPServ, srv2: IPServ) = {
    ...
    newIssues foreach {
      srv1.createIssue_? _ andThen (_ runAsync {
          _ fold (
